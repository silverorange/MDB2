...
$stmt = $mdb2->prepare('INSERT INTO numbers VALUES(?,?,?)', ['integer', 'text', 'text'], MDB2_PREPARE_MANIP);
...
echo var_dump($mdb2->extended->getOne('SELECT trans_en FROM numbers WHERE number = ?', null, $array, ['integer'])) . '<br>';
$mdb2->setFetchmode(MDB2_FETCHMODE_ASSOC);
echo '<br>default fetchmode ist now MDB2_FETCHMODE_ASSOC<br>';
echo '<br>see getRow in action:<br>';
echo var_dump($mdb2->extended->getRow('SELECT * FROM numbers WHERE number = ?', ['integer', 'text', 'text'], $array, ['integer']));
...
echo var_dump($mdb2->extended->getCol('SELECT * FROM numbers WHERE number != ?', null, $array, ['integer'], 1)) . '<br>';
echo '<br>see getAll in action:<br>';
echo var_dump($mdb2->extended->getAll('SELECT * FROM test WHERE test_id != ?', ['integer', 'text', 'text'], $array, ['integer'])) . '<br>';
echo '<br>see getAssoc in action:<br>';
echo var_dump($mdb2->extended->getAssoc('SELECT * FROM test WHERE test_id != ?', ['integer', 'text', 'text'], $array, ['integer'], MDB2_FETCHMODE_ASSOC)) . '<br>';
